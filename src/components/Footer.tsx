    ...
    <footer className="bg-navy text-white bb-pattern">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div><p className="section-label text-white/45 mb-4">{lang === 'tr' ? 'Kategoriler' : 'Categories'}</p><ul className="space-y-2">{[{ slug: 'kisisel-gelisim', emoji: '🧠' },{ slug: 'tekstil', emoji: '🧵' },{ slug: 'turkiye-gundemi', emoji: '🇹🇷' }].map(cat => <li key={cat.slug}><Link href={`/${lang}/blog?category=${cat.slug}`} className="text-sm text-white/70 hover:text-accent-blue transition-colors flex items-center gap-2"><span>{cat.emoji}</span><span>{t(`cat.${cat.slug}` as any)}</span></Link></li>)}</ul></div>
