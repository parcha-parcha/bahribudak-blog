    ...
    <footer className="bg-navy text-white bb-pattern">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div><div className="mb-5 rounded-[26px] bg-white/95 p-3 md:p-4 inline-flex items-center justify-center"><BrandLogo className="h-[86px] w-[330px] max-w-full" /></div><p className="text-sm text-white/65 leading-relaxed">{t('footer.tagline')}</p><div className="mt-5 w-10 h-0.5 bg-accent-blue" /></div>
          <div><p className="section-label text-white/45 mb-4">{lang === 'tr' ? 'Kategoriler' : 'Categories'}</p><ul className="space-y-2">{[{ slug: 'tekstil' },{ slug: 'kisisel-gelisim' },{ slug: 'turkiye-gundemi' }].map(cat => <li key={cat.slug}><Link href={`/${lang}/blog?category=${cat.slug}`} className="text-sm text-white/70 hover:text-accent-blue transition-colors flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-accent-blue" /><span>{t(`cat.${cat.slug}` as any)}</span></Link></li>)}</ul></div>
