    ...
    <footer className="bg-navy text-white bb-pattern">
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div><div className="mb-5 rounded-2xl bg-white/95 p-4 inline-block"><BrandLogo className="h-14 w-[225px] text-navy" /></div><p className="text-sm text-white/65 leading-relaxed">{t('footer.tagline')}</p><div className="mt-5 w-10 h-0.5 bg-accent-blue" /></div>
          <div><p className="section-label text-white/45 mb-4">{lang === 'tr' ? 'Kategoriler' : 'Categories'}</p><ul className="space-y-2">{['tekstil', 'kisisel-gelisim', 'turkiye-gundemi'].map(slug => <li key={slug}><Link href={`/${lang}/blog?category=${slug}`} className="text-sm text-white/70 hover:text-accent-blue transition-colors flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-accent-blue/80" /><span>{t(`cat.${slug}` as any)}</span></Link></li>)}</ul></div>
