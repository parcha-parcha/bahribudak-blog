    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-border">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7">
          {navLinks.map(link => {
            ...
            return (
              <Link key={link.href} href={link.href} className={`text-sm font-semibold transition-colors ${isActive ? 'text-navy border-b-2 border-yellow-bb pb-0.5' : 'text-gray-text hover:text-navy'}`}>
                {link.label}
              </Link>
            )
          })}
        ...
        <div className="flex items-center gap-3">
          <Link href={switchedPath} className="text-xs font-bold uppercase tracking-widest text-gray-text hover:text-navy border border-gray-border rounded-full px-3 py-1.5 transition-colors hover:border-navy">
            {otherLang.toUpperCase()}
          </Link>
          <a href="https://www.linkedin.com/in/bahri-budak-052ab5b8" target="_blank" rel="noopener noreferrer" className="hidden sm:flex items-center gap-1.5 bg-navy text-white text-xs font-bold px-4 py-2 rounded-full hover:bg-navy-light transition-colors">
            LinkedIn
          </a>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-gray-border px-6 py-4 space-y-4">
          {navLinks.map(link => (
            <Link key={link.href} href={link.href} className="block text-sm font-semibold text-navy hover:text-yellow-bb transition-colors" onClick={() => setMenuOpen(false)}>
              {link.label}
            </Link>
          ))}
        </div>
