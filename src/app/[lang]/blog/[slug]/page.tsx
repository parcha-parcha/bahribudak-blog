...
import ArticleSchema from '@/components/ArticleSchema'  // ← YENİ
...
export async function generateMetadata({ params }: PostPageProps): Promise<Metadata> {
  ...
  return {
    ...
    // ← YENİ: Open Graph (LinkedIn, WhatsApp paylaşımlarında önizleme gösterir)
    openGraph: {
      title: post.title,
      description: post.excerpt,
      url: postUrl,
    ...
    // ← YENİ: Canonical URL — duplicate content'i önler
    alternates: {
      canonical: postUrl,
    },
  }
...
export default async function PostPage({ params }: PostPageProps) {
  ...
  return (
    <>
      {/* ← YENİ: JSON-LD Article Schema — Google'ın yazıyı tanıması için */}
      <ArticleSchema
        title={post.title}
      ...
      <article className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-text mb-12 pb-8 border-b border-gray-border">
          ...
          <time dateTime={new Date(post.date).toISOString()}>
            {new Date(post.date).toLocaleDateString(lang === 'tr' ? 'tr-TR' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
          </time>
          <span>·</span>
