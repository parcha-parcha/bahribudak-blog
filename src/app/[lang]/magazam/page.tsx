...
export async function generateMetadata({ params }: { params: { lang: string } }): Promise<Metadata> {
  ...
  return {
    title: tr ? 'Tekstil Kaynak Merkezi | Bahri Budak' : 'Textile Resource Center | Bahri Budak',
    description: tr
  ...
  return (
    <article
      className={`group rounded-[2rem] border bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-card ${
        item.featured ? 'border-accent-blue/55 ring-1 ring-accent-blue/20' : 'border-gray-border'
      }`}
    >
      <div className="mb-6 flex items-start justify-between gap-5">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div>
            <p className="mb-2 text-xs font-black uppercase tracking-[0.16em] text-navy/55">{item.status}</p>
            <h2 className="text-2xl font-bold leading-tight text-navy">{item.title}</h2>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <p className="mb-1 text-xs font-black uppercase tracking-[0.16em] text-accent-blue">Dosya amacı</p>
          <p className="text-[15px] leading-relaxed text-navy/78">{item.purpose}</p>
        </div>

        <div className="rounded-2xl border border-gray-border bg-[#F3F6FA] px-4 py-3">
          <p className="mb-1 text-xs font-black uppercase tracking-[0.16em] text-navy/55">Kim kullanır?</p>
          <p className="text-sm leading-relaxed text-navy/76">{item.users}</p>
        </div>

        <div>
          <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-accent-blue">İçerik örnekleri</p>
          <div className="flex flex-wrap gap-2">
            {item.examples.map((example) => (
              <span key={example} className="rounded-full border border-gray-border bg-white px-3 py-1 text-xs font-semibold text-navy/76">
                {example}
              </span>
            ))}
          </div>
      ...
      <section className="relative overflow-hidden bg-[#061A33] text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-16 md:py-20">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white">Tekstil dokümanları</span>
            <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white">Proses formları</span>
            <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white">Kontrol listeleri</span>
            <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white">Eğitim notları</span>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="rounded-[2rem] border border-gray-border bg-white p-7 shadow-sm md:p-9">
          <div className="mb-7 max-w-3xl">
            ...
            <h2 className="text-3xl font-bold text-navy">Kurumsal ve dijital şablonlar ana ürün değildir</h2>
            <p className="mt-3 leading-relaxed text-navy/72">
              Kurumsal evrak ve dijital yayın şablonları, tekstil teknik dosyalarının daha profesyonel sunulması için isteğe göre hazırlanır. Bu bölümün ana önceliği tekstil teknik bilgi dosyalarının indirilebilir hale getirilmesidir.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
